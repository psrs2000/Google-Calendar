...
def obter_config_secret(chave, padrao=""):
    """Busca configuração com prioridade: 1º Banco, 2º Secrets, 3º Padrão"""
    
    # 1º: Tentar buscar no banco local primeiro
    try:
        valor_banco = obter_configuracao(chave, None)  # None = não encontrado
        if valor_banco is not None and str(valor_banco).strip():
            return valor_banco
    except:
        pass  # Se der erro, continua para secrets
    
    # 2º: Se não encontrou no banco, tentar secrets (WEB)
    try:
        valor_secrets = st.secrets.get("configuracoes", {}).get(chave, None)
        if valor_secrets is not None and str(valor_secrets).strip():
            return valor_secrets
    except:
        pass  # Se der erro, usar padrão
